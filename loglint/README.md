...
> Примечание: для Go Plugin System требуется собирать `golangci-lint` и плагин под одну и ту же платформу и с `CGO_ENABLED=1`. Для учебного проекта обычно достаточно локальной сборки и запуска на одной машине.
...
Таким образом, сборка и прогон тестов анализатора выполняются автоматически при изменениях в репозитории.
